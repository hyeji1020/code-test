...
public class ProductController {
     ...
     * 2. 기능상 문제는 없지만, URI에 @PathVariable로 ID 값을 받아 어떤 객체를 수정하는지 명시해주는게 직관적임.
     ...
     *
     * ps.
     * 요청 DTO에 필터링 데이터와 페이징 데이터를 함께 담는 구조에 대해 고민했습니다.
     * 필터링 DTO에 페이징 정보를 포함할 경우, 다른 도메인의 페이징 구현 시 중복된 페이징 필드가 반복될 가능성이 높습니다.
     * 따라서 별도의 페이징 전용 DTO를 만들거나, Spring Data의 표준 인터페이스(Pageable)를 활용하는 것이 적절할 것 같습니다.
     * 현재 프로젝트가 Spring Data JPA 기반이므로, 일관성을 위해 페이징은 @PageableDefault로 처리하고,
     * 필터링은 별도 DTO로 관리하는 방식이 더 적합하다고 판단했습니다.
